...
```bash
pip install stateloom[semantic]   # guardrails NLI, semantic cache
pip install stateloom[prompts]    # file-based agents (watchdog)
pip install langchain langchain-openai  # LangChain/LangGraph
pip install litellm               # LiteLLM
pip install google-genai           # google-genai
...
|--------|---------|:-------:|
| `quick_start.py` | Auto-patching OpenAI/Anthropic/Gemini SDKs, session tracking, per-model cost breakdown | Yes |
| `unified_client.py` | Provider-agnostic `stateloom.chat()` and `Client` — no SDK imports needed | Yes |
| `sessions_and_budget.py` | Session lifecycle, per-session budget enforcement, parent-child hierarchy | Yes |
| `pii_detection.py` | PII scanning modes (audit, redact, block), custom rules, GDPR compliance profiles | Yes |
| `guardrails.py` | Prompt injection & jailbreak detection (32 heuristic patterns), audit vs enforce mode, NLI classifier | Yes |
| `caching.py` | Exact-match request caching, cache hit tracking, cost savings | Yes |
| `local_models.py` | Ollama integration, hardware detection, model recommendations, auto-routing | No* |
| `durable_resumption.py` | Crash-recovery checkpointing (Temporal-like), durable sessions, resume from cached steps | Yes |
| `semantic_retries.py` | Self-healing LLM output: `retry_loop()` for bad JSON, `durable_task()` decorator | Yes |
| `time_travel.py` | Record-replay debugging, session export/import, VCR-cassette mock | Yes |
| `agents_and_consensus.py` | Managed agents (Prompts-as-an-API), multi-agent consensus (vote, debate, self-consistency) | Yes |
| `experiments.py` | A/B experiments: variant assignment, model/prompt overrides, feedback, metrics, leaderboard | Yes |
| `kill_switch.py` | Global & granular emergency stop: model globs, provider blocks, error vs response mode | Yes |
| `langchain_langgraph.py` | LangChain chains + LangGraph ReAct agents with full middleware pipeline | Yes |
| `litellm.py` | LiteLLM multi-provider routing with cost tracking, budget, guardrails, streaming | Yes |
| `zero_trust_security.py` | Secret vault (environ scrub), CPython audit hooks (PEP 578), layered defense | Yes |
| `circuit_breaker.py` | Per-provider circuit breaker, tier-based fallback, manual reset, layered protection | Yes |
| `compliance.py` | GDPR/HIPAA/CCPA profiles, tamper-proof audit trail, Right to Be Forgotten, custom profiles | Yes |
| `model_testing.py` | Dark launch candidates alongside production, similarity scoring, sampling, smart skip logic | Yes* |
| `proxy_gateway.py` | OpenAI-compatible proxy with PII, guardrails, caching, kill switch, dashboard API queries | Yes |
| `google_genai.py` | New google-genai SDK: generate_content, streaming, multi-turn, budget enforcement | Yes |
| `cohere.py` | Cohere V2 SDK: chat, chat_stream, multi-turn, budget enforcement | Yes |
| `mistral.py` | Mistral SDK: chat.complete, chat.stream, multi-turn, budget enforcement | Yes |

\* `local_models.py` requires a running [Ollama](https://ollama.com) instance instead of a cloud API key. `model_testing.py` requires two provider API keys for cross-provider comparison.
...
### Quick Start
Auto-patches installed SDKs so native `openai.OpenAI()`, `anthropic.Anthropic()`, `genai.GenerativeModel()` (legacy), and `google.genai.Client()` (new SDK) calls flow through the middleware pipeline. Shows per-model cost/token breakdown in a single session.
...
Auto-patches the new `google-genai` SDK (`google.genai.Client()`). Covers `generate_content` (non-streaming), `generate_content_stream` (streaming), multi-turn conversations with Content dicts, and budget enforcement. Both sync (`Models`) and async (`AsyncModels`) are patched. The new SDK uses per-client API keys and keyword-only arguments — StateLoom handles both this and the legacy `google-generativeai` SDK simultaneously.

### Cohere
Auto-patches Cohere's V2 SDK (`cohere.ClientV2().chat()` and `chat_stream()`). Covers basic chat, multi-turn conversations, streaming via `chat_stream()` with full middleware coverage (cost tracking, PII, guardrails), and budget enforcement.

### Mistral
Auto-patches the Mistral SDK (`Mistral().chat.complete()` and `chat.stream()`). Covers basic chat, multi-turn conversations, streaming via `chat.stream()` with full middleware coverage (cost tracking, PII, guardrails), and budget enforcement.
